...
class Boolean_Network_App(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        tk.Tk.wm_title(self,"Boolink") # change title
    ...
    def show_frame(self, cont):
        ...
        if frame.getType() == "GraphPage":
            popupmsg("Make sure you have set nodes to see their graphical result\nAlways hit the refresh button before reading, since many things are related\nThat's it, everytime you run a simulation, adjust some specific nodes settings, and are ready to read the graph, hit refresh first.")
...
class NetDropDown(tk.Tk):
    ...
    # this function can also modefy a node's state
    def AddNode(self):
        s = self.entry_new.get() # what ever the user put into the input box just before they click add
...
class StartPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label = tk.Label(self, text=("""Boolink - Boolean Network GUI Application
        use at your own risk. There is no promise of warranty
        
        Instructions: 
        1. Simulate after setting nodes and boolean equations
        2. Once you add nodes or eqns, the whole network is destroyed and started over, so change both nodes and eqns
        3. When running the simulation, please be patient. While it is running, the application will freeze. After it is done, you can see results"""), font=LARGE_FONT)
